...
import { IconPlus, IconBookmark, IconUnlink, IconTrash } from '@tabler/icons-vue'
...
async function saveSelected() {
  ...
  const name = window.prompt('Nama komponen baru?', 'Komponen')
  if (!name?.trim()) return
...
function del(id: number) {
  if (confirm('Hapus komponen master? Instance yang ada tidak akan bisa resolve.')) {
...
<template>
  <div class="border-t border-neutral-200 p-2">
    <div class="mb-1.5 flex items-center justify-between">
      <span class="text-xs font-medium text-neutral-500">Komponen</span>
      <button
        class="rounded p-1 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-700 disabled:opacity-30"
        :disabled="!canSave()"
        title="Simpan node terpilih sebagai komponen"
        @click="saveSelected"
      >
    ...
    <button
      ...
      @click="breakLink"
    >
      <IconUnlink class="size-3.5" /> Putus link (jadikan copy)
    </button>
    ...
    <div class="space-y-1">
      ...
      >
        <button
          ...
          :title="`Sisipkan instance ${c.name}`"
          @click="insert(c.id)"
        >

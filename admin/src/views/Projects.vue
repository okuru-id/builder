      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow v-else-if="projects.length === 0">
            <TableCell :colspan="4" class="text-center text-muted-foreground">No projects.</TableCell>
          </TableRow>
          <TableRow v-for="p in projects" :key="p.id">
            <TableCell class="font-medium">{{ p.title_en }}</TableCell>
            <TableCell>{{ p.sort_order }}</TableCell>
            <TableCell>
              <Badge v-if="p.featured" variant="default">featured</Badge>
              <span v-else class="text-muted-foreground">—</span>
            </TableCell>
            <TableCell class="text-right">
              <div class="flex justify-end gap-1">
                <Button variant="ghost" size="icon-sm" @click="startEdit(p)">
                  <IconEdit class="size-4" />
                </Button>
                <Button variant="ghost" size="icon-sm" @click="remove(p.id)">
